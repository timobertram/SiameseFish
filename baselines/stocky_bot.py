...
class StockyInference(Player):
    def __init__(self):
        ...
        if STOCKFISH_ENV_VAR not in os.environ:
            raise KeyError(
                'StockyInference requires an environment variable called "{}" pointing to the Stockfish executable'.format(
                    STOCKFISH_ENV_VAR))
        ...
        for k in brds:
            ...
            if taken_move is not None:
                if taken_move in tBoard.pseudo_legal_moves:
                    if (captured_opponent_piece == True) and (tBoard.piece_at(capture_square) is not None) and (
                            tBoard.piece_at(capture_square).piece_type != chess.KING):
                ...
                if (tBoard.piece_at(requested_move.from_square) is not None) and (
                        tBoard.piece_at(requested_move.from_square).piece_type == chess.PAWN):
                    if tBoard.piece_at(requested_move.to_square) is None and (
                            chess.square_file(requested_move.from_square) != chess.square_file(
                            requested_move.to_square)):
                        newBoards[k] = brds[k].copy()
                        ...
                        if abs(chess.square_rank(requested_move.from_square) - chess.square_rank(
                            ...
                            if tBoard.piece_at(chess.square(chess.square_file(requested_move.to_square), (
            ...
            while (cN) < (len(self.opponent_move_results)):
                ...
                if cN != -1:
                    ...
                    if cN < (len(self.my_move_results)):
                        ...
                        brds = self.moveResultInference(requested_move, taken_move, captured_opponent_piece,
                                                        capture_square, brds)
                cN = cN + 1
    ...
    def handle_move_result(self, requested_move: Optional[chess.Move], taken_move: Optional[chess.Move],
        ...
        if captured_opponent_piece and self.debug:
            print("I killed something")
        newBoards = {}
        self.potBoards = self.moveResultInference(requested_move, taken_move, captured_opponent_piece, capture_square,
                                                  self.potBoards)
        if not self.won:
            self.recover()
        ...
        if self.debug:
            print("Considering ", len(self.potBoards), " potential board states || Earliest Checkpoint: ",
                  self.checkpoints[0][0] if len(self.checkpoints) != 0 else "---")
            print("StockyInf.:\t End move ", self.moveNum, " -- approx ", '{:06.2f}'.format(self.approxTime),
                  " seconds left || Score: ", '{:06.1f}'.format(self.score))
