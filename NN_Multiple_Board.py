...
import os
import chess.svg
from create_dataset import process_file_win
import random
import numpy as np
...
class NN_MultipleBoard(Player):

    def __init__(self,eval_path = 'networks/stockfish/tuned.pt',output_path = None, randomized = False, normal = True):
        ...
        if normal:
            ...
            if torch.cuda.is_available():
                memory_usage = {d:torch.cuda.mem_get_info(torch.device(d))[0] for d in ['cuda:0','cuda:1','cuda:2','cuda:3']}
                print(memory_usage)
                self.device = max(memory_usage, key = memory_usage.get)
            else:
                self.device = 'cpu'
            print(f'Will use device {self.device}')

            self.evaluation_network = utils.load_network(eval_path).to(self.device)
        else:
            self.siamese = SiameseAgent(device = 'cpu')
            self.device = 'cpu'
    ...
    def handle_game_start(self, color: Color, board: chess.Board, opponent_name: str):
        ...
        self.color = color
        self.track_turn = play_utils.Track_turn(self.color)
        if self.debug:
        ...
        if self.randomized:
            self.num_random = 10
            print(f'Will play first {self.num_random} moves randomized')
                    ...
                    for move in list(board.pseudo_legal_moves):
                        ...
                        if board.piece_at(move.to_square) is None and not board.is_en_passant(move):
                            ...
                            if new_fen not in fens:
                                ...
                                fens.append(new_fen)
                    # castling:
                    for board in play_utils.illegal_castling(board, self.color):
    ...
    def get_evaluations_of_subset(self, all_values, boards, move):
       ...
       # weighted_sum /= num_boards
        ...
        return weighted_sum,weighted_sum,weighted_worst_eval

    def get_variance_of_subset(self, all_values, boards, move):
        board_dict = all_values[move]
        worst_eval = 1
        best_eval = 0
        for board in boards:
    ...
    def sense_variance_reduction(self, sense_actions: List[Square], move_actions: List[chess.Move], seconds_left: float) \
        -> Optional[Square]:
        start_time = time.time()
        ...
        for i,square in enumerate(elimination_chances):
            for resulting_boards,prob in square:
                variance = self.get_variance_of_subset(all_values,resulting_boards,best_move)
                square_values[i] -= prob*variance
        #Output            
        if sum(square_values) == 0 and self.board_dict.size() != 1:
            print(elimination_chances)
    ...
    def choose_sense(self, sense_actions: List[Square], move_actions: List[chess.Move], seconds_left: float) \
        ...
        square_info_gains,square_board_reduction = self.sense_variance_reduction(sense_actions,move_actions,seconds_left)

        print(f'Best info gain square {np.argmax(square_info_gains)}')
        if np.argmax(square_info_gains) == 0:
            print(f'Full info gain vector: {square_info_gains}')
    ...
    def is_checkmate(self,board,move):
        enemy_king_square = board.king(not self.color)
        # if there are any ally pieces that can take king, execute one of those moves
        enemy_king_attackers = board.attackers(self.color, enemy_king_square)
        if enemy_king_attackers:
            if move in [chess.Move(attack_square, enemy_king_square) for attack_square in enemy_king_attackers]:
                return True
        return False


    def get_evaluations_of_board(self,board,move_options):
        inputs = []
        wins = []
        for i,move in enumerate(move_options):
            inp_board = copy.deepcopy(board)
            if move:
                if self.is_checkmate(board,move):
                    wins.append(i)
                new_move = rbc.utilities.revise_move(board,move)
            else:
                new_move = move
            ...
            if new_move is not None:
                inp_board.push(new_move)
            else:
                inp_board.push(chess.Move.null())
            if inp_board.turn:
                turn_tensor = torch.ones((1,8,8), device = self.device)
            else:
                turn_tensor = torch.zeros((1,8,8), device = self.device)
            capture_tensor = torch.zeros((1,8,8), device = self.device)
            if new_move is not None and board.piece_at(new_move.to_square) is not None and board.piece_at(new_move.to_square).color != self.color:
                row,col = utils.last_move_to_capture_square(new_move)
                capture_tensor[0,row,col] = 1 
            inp = torch.cat((utils.board_from_fen(inp_board.fen()).to(self.device),capture_tensor,turn_tensor),dim = 0)
            inputs.append(inp)

        input = torch.stack(inputs)
        output = torch.sigmoid(self.evaluation_network(input).squeeze())
        if self.color:
            evaluations = output.tolist()
        else:
            evaluations = (torch.ones_like(output)-output).tolist()
        for win in wins:
            evaluations[win] = 1
        return evaluations
    ...
    def evaluate_moves(self,weighted_boards,move_options,columns, last_results = None):
        ...
        min_boards = 5
        num_boards = len(weighted_boards)
        boards_to_evaluate = max(num_boards//5,min_boards)
        for i,(board,weight) in enumerate(weighted_boards[:boards_to_evaluate]):
            reduced_fen = self.reduce_fen(board.fen())
            ...
            if last_results and reduced_fen in last_results['weight'].keys():
                list_values = [last_results[move][reduced_fen] for move in move_options]
            else:
                list_values = self.get_evaluations_of_board(board,move_options)
                if not list_values:
                    list_values = self.get_evaluations_of_board(board,move_options)

            if reduced_fen in columns.keys():
                old_series = columns[reduced_fen].tolist()
                columns[reduced_fen] = pd.Series(old_series[:-1]+['']+list_values+[weight])
            else:
                columns[reduced_fen] = pd.Series(list_values + [weight])

            for j,move in enumerate(move_options):
                all_values[reduced_fen][move] = list_values[j]
            all_values[reduced_fen]['weight'] = weight
    ...
    def choose_move_board_evaluation(self, move_actions: List[chess.Move], seconds_left: float) -> Optional[chess.Move]:# uses a list representation of our board_dict
        play_utils.equal_board_check(self.board_dict.get_boards(), self.color)
        start_time = time.time()
        weighted_boards = self.reduce_boards(200)
        ...
        if self.randomized:
            options = [m[0] for m in sorted_votes]
            probabilities = [m[1][0] for m in sorted_votes]
            normalized_probabilities = [p/sum(probabilities) for p in probabilities]
            chosen_move = np.random.choice(options,p = normalized_probabilities)
        else:
            chosen_move = sorted_votes[0][0]
        if chosen_move != None and chosen_move.promotion != None:
            chosen_move.promotion = chess.QUEEN
        elif chosen_move == None:
    ...
    def choose_move(self, move_actions: List[chess.Move], seconds_left: float) -> Optional[chess.Move]:
        ...
        self.timer.sync_timeout(seconds_left)
        if self.randomized and self.get_turn_number() > self.num_random:
    ...
    def reduce_boards(self,max_num,delete = False, get_weighting = True):
        ...
        weighting,_,_ = self.siamese.get_board_weightings(possible_boards)
    ...
    def handle_game_end(self, winner_color: Optional[Color], win_reason: Optional[WinReason], game_history: GameHistory):
        try:
            if self.output_path:
                process_file_win(history = game_history, outpath_win = self.output_path)
            del self.siamese
            if self.normal:
                del self.evaluation_network
                self.engine.quit()
                self.engine.close()
                del self.engine
